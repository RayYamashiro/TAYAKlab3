...
public class PDA {
    ...
    static class LeftPos {
        ...
        public LeftPos(char s, char p, char h){
            this.s = s;
            this.p = p;
            this.h = h;
        }
    ...
    static class RightPos {
        ...
        public RightPos(char s, String c){
            this.s = s;
            this.c = c;
        }
    }
    ...
    static class Command {
        public LeftPos fargs;
        public ArrayList<RightPos> values;

        public Command(LeftPos f, ArrayList<RightPos> v){
            this.fargs = f;
            this.values = v;
        }
    }
    ...
    static class ComBuilder {
        ...
        List<Configuration> chain = new ArrayList<>(); // цепочка конфигураций магазинного автомата
        ...
        public ComBuilder(String filename) throws IOException {
            ...
            while (tmpStr != null && !tmpStr.equals("")) {
                ...
                if (!matcher.matches() ||
                    ...
                    commands.add(new Command(
                            new LeftPos(s0, emptySym, matcher.group(1).charAt(0)),
                            new ArrayList<>()));
                    commands.get(commands.size() - 1).values.add(new RightPos(s0, ""));
            ...
            for (Character c : terminals) {
                commands.add(new Command(new LeftPos(s0, c, c), new ArrayList<>(){{
                    add(new RightPos(s0, "\0"));
                }}));
            }
            commands.add(new Command(new LeftPos(s0, emptySym, h0), new ArrayList<>(){{
                add(new RightPos(s0, "\0"));
            }}));
        }
        ...
        void printChain() {
            ...
            System.out.print("(s0, lambda, lambda) -|");
    ...
    public static void main(String[] args) {
        ...
        String str;
        try {
            ComBuilder strg = new ComBuilder("src/test1");
            strg.printData();
            while (true)
            {
